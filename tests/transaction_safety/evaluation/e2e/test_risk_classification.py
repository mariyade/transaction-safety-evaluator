...
def _binary(results):
    """Convert to binary risk labels. FLAGGED/ESCALATE count as caught risk."""
    y_true, y_score = [], []
    for r in results:
        if r["expected"] not in ("SAFE", "FLAGGED"):
            continue
        if r["predicted"] not in ("SAFE", "FLAGGED", "ESCALATE"):
            continue
        y_true.append(1 if r["expected"] == "FLAGGED" else 0)
        if r["predicted"] == "ESCALATE":
            y_score.append(1.0)
        elif r["predicted"] == "FLAGGED":
            y_score.append(r["confidence"])
        else:
            y_score.append(1 - r["confidence"])
