...
from agents.transaction_safety.pydantic_validator import (
    create_retry_prompt,
...
class TestValidateResponseWithRetries:
    def test_valid_existing_response_does_not_call_llm(self):
        def fail_if_called(_):
            raise AssertionError("LLM should not be called for an already-valid response")

        result, error = validate_response_with_retries(
            response_content=VALID_RESPONSE,
            retry_prompt_context="evaluate this",
            data_model=AddressValidationResult,
            call_llm_fn=fail_if_called,
            n_retry=3,
        )
