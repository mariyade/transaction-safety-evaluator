...
def test_execute_tool_call_validates_and_dispatches_assess_risk():
    result = tools.execute_tool_call(_tool_call(
        "assess_risk",
        {
            "address": "unknown",
            "chain": "unknown",
...
def test_execute_tool_call_returns_unknown_tool_message():
    result = tools.execute_tool_call(_tool_call("not_a_tool", {}))

    assert result == "Unknown tool: not_a_tool"


def test_execute_tool_call_raises_for_invalid_tool_args():
    with pytest.raises(ValidationError):
        tools.execute_tool_call(_tool_call("assess_risk", {"chain": "ethereum"}))
